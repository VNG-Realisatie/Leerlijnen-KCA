...
* In `<ordergegevens>` plaats je in een sequence de al in het schema voorkomende elementdefinitie van elementen `<ordernummer>` en creëer je een elementdefinitie voor het element `<artikel>`. Deze krijgen allen het datatype 'xs:string' behalve de laatste, die krijgt het datatype 'xs:positiveInteger';


* In artikel creëer je wederom in een sequence de elementdefinities voor `<artikelnummer>`, `<productnaam>`, `<omschrijving>` en `<aantal>`;
