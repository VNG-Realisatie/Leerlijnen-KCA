...
* Doe met de twee andere `<klantgegevens>` elementen in het XML-Schema hetzelfde. Daarvoor moet je daar het `<xs:complexType>` verwijderen;
* Bewaar het bestand;
...
***Herhaal de techniek***
* Kijk nu of je dezelfde techniek als hierboven los kunt laten op de elementen `<naam>`, `<adres>`, `<ordergegevens>`, `<artikel>` en `<leveringsgegevens>` maar **LET OP!** vervang de inhoud van de elementen `<ordergegevens>` en `<artikel>` binnen `<factuur>` nog niet. 
...
* Bewaar het bestand, bewaar het ook meteen als “Procesdocumentatie5.xsd” en bekijk de root elementen weer in Schema view.
